...
CONTAINS
   ...
   SUBROUTINE radconst(secs_into_sol,DECLIN,SOLCON,JULIAN,time_of_day,           &
                       DEGRAD,DPD,                                   &
...
!---------------------------------------------------------------------
   ...
   USE module_model_constants, ONLY : planet_year, solar_constant
   IMPLICIT NONE
!---------------------------------------------------------------------

! !ARGUMENTS:
   REAL, INTENT(IN   )      ::       DEGRAD,DPD,secs_into_sol,JULIAN,time_of_day!, gmt
...
!c-mm            sza0(i,j)=MAX(REAL(ss+cc*SIN(ha1)/ha1),0.)
!c-mm  I'm basically doing a hard overwrite of what was being done above for
!c-mm  use in the 1-D RT model.  Basically, I want to calculate ha
!c-mm  from just knowing time_of_day.  ha=0 at noon, and then grows
!c-mm  accordingly until it's pi at midnight.
!c-mm  It's possible, though, that the original implementation had ha=0
!c-mm  at midnight, because I did have to change SIN(ha1) to
!c-mm  COS(ha1) in the equation below.
            
            ha1=(time_of_day-12.)*pi/12.
            ha(i,j)=REAL(ha1)
            sza0(i,j)=MAX(REAL(ss+cc*COS(ha1)),0.)
         end if
      END DO
   END DO
