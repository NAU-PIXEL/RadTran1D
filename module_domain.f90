...
#if ( WRF_PLANET == 1 )
   ! The limit on the number of variables that can be changed in the
   ! "iofields_filename" file when setting variables for an input/output stream
   INTEGER, PARAMETER, PRIVATE :: MAX_DEXES = 1000
#endif
...
#if ( WRF_PLANET == 1 )
      LOGICAL, SAVE      :: you_warned_me2(MAX_DEXES,max_domains) = .FALSE.
#else
...
! If not, satisfy the request
...
#if ( WRF_PLANET == 1 )
     LOGICAL,       INTENT(INOUT) :: you_warned_me2(MAX_DEXES,max_domains)
     CHARACTER*256                :: string_max_dexes
#else
   ...
   ! Local
     ...
     ENDIF
#if ( WRF_PLANET == 1 )
     IF ( count_em > MAX_DEXES ) THEN
#else
     IF ( count_em > 100 ) THEN
#endif
#if ( DA_CORE != 1 )
#if ( WRF_PLANET == 1 )
       WRITE(string_max_dexes,'(I0)') MAX_DEXES
       WRITE(mess,*)'ERROR module_domain:  Array size for you_warned_me2 is fixed at '//TRIM(string_max_dexes)
       CALL wrf_message(mess)
       CALL wrf_error_fatal('Did you really type > '//TRIM(string_max_dexes)//' fields into '//TRIM(fname)//' ?')
#else
...
#if ( WRF_PLANET == 1 )
        REAL,                    INTENT(  OUT), OPTIONAL :: seconds_into_sol
#endif
! <DESCRIPTION>
...
! </DESCRIPTION>
        ...
        ENDIF
#if ( WRF_PLANET == 1 )
        IF ( PRESENT( seconds_into_sol ) ) THEN
          lcl_currtime = domain_get_current_time ( grid )
          CALL WRFU_TimeGet( lcl_currtime, H=hours, M=minutes, S=seconds, Sn=Sn, Sd=Sd, rc=rc )
          seconds_into_sol = real(seconds) + (real(hours)*60.*60.) + (real(minutes)*60.)
          IF ( Sd /= 0 ) THEN
            seconds_into_sol = seconds_into_sol + ( REAL( Sn ) / REAL( Sd ) )
          ENDIF
        ENDIF
#endif
        RETURN
...
#if ( WRF_PLANET == 1 )
        CALL domain_clock_get( grid, seconds_into_sol=grid%secs_into_sol )
#endif
...
#if ( WRF_PLANET == 1 )
     CHARACTER*256 mess
     write(mess,*)'modify_io_masks head_grid ',id,ASSOCIATED(head_grid)
     call wrf_debug(500,mess)
#endif
!write(0,*)'modify_io_masks head_grid ',id,ASSOCIATED(head_grid)
     CALL find_grid_by_id( id, head_grid, grid )
#if ( WRF_PLANET == 1 )
     write(mess,*)'modify_io_masks grid ',id,ASSOCIATED(grid)
     call wrf_debug(500,mess)
#endif
